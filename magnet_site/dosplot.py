...
def create_dosplot(compound, atoms_per_unit_cell):
    path = j(DOS, compound)
    df1 = pd.read_csv(
        j(path, "nonsp_dost.dat"), names=["energy", "dos", "idos"], sep="\s+"
    )
    df1["type"] = "nonsp"
    df2 = pd.read_csv(
        j(path, "sp_dost.dat"), names=["energy", "udos", "ddos"], sep="\s+"
    )
    df2["type"] = "sp"
    df2["ddos"] = -df2["ddos"]
    ...
    p = figure(
        x_axis_label=r"E - Ef (eV)",
        y_axis_label="density of states (states/eV atom)",
    ...
    p_nonsp = p.line(
        "energy",
        "dos",
        legend_label="No spin polarization",
        line_color="#606060",
        line_dash="solid",
    ...
    hover_nonsp = HoverTool(
        tooltips=[("Energy", "@energy"), ("DOS", "@dos")], renderers=[p_nonsp]
    )
    ...
    hover_down = HoverTool(
        tooltips=[("Energy", "@energy"), ("Spin-down DOS", "@ddos")], renderers=[p_down]
    )
