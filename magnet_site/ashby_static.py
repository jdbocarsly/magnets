...
def make_payload(df, site_root):
    ...
    fields = list(
        dict.fromkeys(
            field for _, group in resources.axis_columns_groups for field in group
        )
    )
    stats = {
        field: [float(df[field].min()), float(df[field].max())]
        for field in fields
        if field not in resources.discrete_columns
    }
    templates = {}
    for categorical in product((False, True), repeat=3):
        selected = [
            "class" if cat else default for cat, default in zip(categorical, DEFAULTS)
        ]
    ...
    return {
        ...
        "defaults": DEFAULTS,
        "fields": fields,
        "categories": resources.discrete_columns,
        "stats": stats,
    }
