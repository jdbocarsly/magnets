...
from .paths import OUTPUT, ROOT, WEB
...
from .data import MISSING_DOS, counts, display_data, load_data
...
def property_groups(row):
    ...
    for category, fields in resources.dos_columns_groups:
        values = [
            (
                field,
                f"{row[field]:.2f}" if isinstance(row[field], float) else row[field],
            )
            for field in fields
...
def build(output=OUTPUT):
    ...
    for path, site_root in [("index.html", "./"), ("ashby/index.html", "../")]:
        ...
        render(
            ...
            "ashby.html",
            script="",
            div='<div id="ashby-plot"></div>',
            col_groups=resources.axis_columns_groups,
            curr_x=DEFAULTS[0],
            curr_y=DEFAULTS[1],
            curr_c=DEFAULTS[2],
            swap_query=urlencode(
                dict(
                    zip(
                        ["x_axis", "y_axis", "color_axis"],
    ...
    plot.select_one(TapTool).callback = CustomJS(
        code="""
    ...
    for _, row in df.iterrows():
        ...
        display_row["source (experimental)"] = source_links(
            row["source (experimental)"]
        )
        render(
            f"c/{row.cid}/index.html",
            "dos.html",
            script=script,
            div=div,
            formula=row.formula_html,
            dos_columns_groups=property_groups(display_row),
    ...
    for asset in sorted((WEB / "static").rglob("*")):
        if asset.is_file():
            ...
            if (
                ...
                or destination.read_bytes() != asset.read_bytes()
            ):
    ...
    print(
        f"Built {pages} HTML pages in {output}: {total} compounds, {magnetocaloric} magnetocalorics."
    )
