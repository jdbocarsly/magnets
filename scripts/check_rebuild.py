...
from magnet_site.paths import OUTPUT
...
def canonical_html(html):
    ...
    if match:
        ...
        html = re.sub(
            r"const render_items = .*;", "const render_items = <items>;", html
        )
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="magnets-rebuild-") as temporary:
        ...
        for name in manifest:
            if name.endswith(".html"):
                assert canonical_page(OUTPUT / name) == canonical_page(output / name), (
                    name
                )
            else:
    ...
    print(
        "Rebuild matches all 170 pages and copied assets (ignoring incidental Bokeh identifiers/order)."
    )
