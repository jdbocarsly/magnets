...
from magnet_site.paths import DOS, LEGACY_DATA, OUTPUT, ROOT, WEB
...
def baseline_module(filename):
    source = subprocess.check_output(
        ["git", "show", f"{BASELINE}:{filename}"], cwd=ROOT, text=True
    )
...
def decode(value):
    if isinstance(value, dict) and "__ndarray__" in value:
        return np.frombuffer(
            base64.b64decode(value["__ndarray__"]), dtype=value["dtype"]
        ).reshape(value["shape"])
...
class StaticParity(unittest.TestCase):
    ...
    def test_every_axis_and_color_against_original_python(self):
        ...
        for categorical in product((False, True), repeat=3):
            cases.add(
                tuple(
                    "class" if cat else DEFAULTS[i] for i, cat in enumerate(categorical)
                )
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                ["node", str(ROOT / "tests/check_ashby.js"), str(path)], check=True
            )
    ...
    def test_correlation_navigation(self):
        ...
        code = next(
            ref["attributes"]["code"]
            for ref in doc["roots"]["references"]
    ...
    def test_property_precision_and_missing_values(self):
        for _, row in self.df.iterrows():
            ...
            for (_, fields), (_, output) in zip(resources.dos_columns_groups, groups):
                self.assertEqual(
                    [field for field in fields if pd.notna(row[field])],
    ...
    def test_detail_and_correlation_plot_structure(self):
        ...
        old_dos.j = lambda first, *rest: original_join(
            DOS if first == "dos_data" else first, *rest
        )
        ...
        old_corr.cm = types.SimpleNamespace(
            get_cmap=lambda name: matplotlib.colormaps[name]
        )
        for cid in (None, 0, 8, 139):
            path = OUTPUT / (
                "correlations/index.html" if cid is None else f"c/{cid}/index.html"
            )
            ...
            self.assertEqual(
                canonical_document(doc), canonical_document(json_item(plot)["doc"])
            )
    ...
    def test_generated_correlation_and_dos_values(self):
        for cid in [None] + self.df.cid.tolist():
            ...
            if cid is None:
                ...
                for method in ("pearson", "spearman"):
                    ...
                    np.testing.assert_allclose(
                        decode(source[method]), expected, rtol=0, atol=0
                    )
                continue
            row = self.df.loc[self.df.cid == cid].iloc[0]
            ...
            for actual, expected in [
                ...
                np.testing.assert_allclose(
                    decode(actual), expected, rtol=1e-14, atol=1e-14
                )
